...
namespace Kafka.Demo.KSqlConsumer
{
	public class MagicConsumer : BackgroundService
	{
		...
		private async Task ConsumeMessages(CancellationToken stoppingToken)
		{
			...
			{
				consumer.Subscribe("status_updates_new"); //Don't forget this
				...
				while (!stoppingToken.IsCancellationRequested)
				{
					...
					Logger.LogInformation("Received KSqlmessage. {Id}:{Status}.", msgContext.Message.Value.IdNumber, msgContext.Message.Value.Status);
				}
			}
		}
	}
